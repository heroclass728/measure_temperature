...
from src.pylepton.ioctl_numbers import _IOR, _IOW
...
class Lepton(object):
    ...
    def __enter__(self):
        # "In Python 3 the only way to open /dev/tty under Linux appears to be 1) in binary mode and 2) with
        # buffering disabled."
    ...
    def capture_segment(handle, xs_buf, xs_size, capture_buf):
        ...
        # NB: the default spidev bufsiz is 4096 bytes so that's where the 24 message limit comes from: 4096 /
        # Lepton.VOSPI_FRAME_SIZE_BYTES = 24.97... This 24 message limit works OK, but if you really need to optimize
        # the read speed here, this hack is for you:

        # The limit can be changed when spidev is loaded, but since it is compiled statically into newer raspbian
        # kernels, that means modifying the kernel boot args to pass this option. This works too: $ sudo chmod 666
        # /sys/module/spidev/parameters/bufsiz $ echo 65536 > /sys/module/spidev/parameters/bufsiz Then
        # Lepton.SPIDEV_MESSAGE_LIMIT of 24 can be raised to 59
    ...
    def capture(self, data_buffer=None, log_time=False, debug_print=False, retry_reset=True):
        ...
        elif data_buffer.ndim < 2 or data_buffer.shape[0] < Lepton.ROWS or data_buffer.shape[1] < Lepton.COLS or \
        ...
        while True:
            ...
            if retry_reset and (self.__capture_buf[
                                    20, 0] & 0xFF0F) != 0x1400:
                # make sure that this is a well-formed frame, should find line 20 here
